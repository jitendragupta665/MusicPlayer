        ...
        when(requestCode)
        {
            131-> {
                if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED
                        ...
                        && grantResults[4] == PackageManager.PERMISSION_GRANTED){  Handler().postDelayed({
                    val startact = Intent(this@SplashActivity, MainActivity::class.java)
                    startActivity(startact)
                    this.finish()
                }, 1000)
            }else{
                    Toast.makeText(this@SplashActivity,"Please make all the permission to continue",Toast.LENGTH_SHORT).show()
                this.finish()
                }
              return
        }
